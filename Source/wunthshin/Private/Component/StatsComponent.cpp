...
{
    // 임시코드, 나중에 parsing하면 바꿔야됨
    FProperty* foundStat = FindFieldChecked<FProperty>(FCharacterStats::StaticStruct(), FName(InStatName));
    foundStat->CallSetter(&CurrentStats, &InStatIncreasement);
}
