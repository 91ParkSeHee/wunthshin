...
void UWSServerSubsystem::TryChangeCharacterStatus(const FString& InStatName, const int32 InStatIncreasement)
{
	CharacterStatusMessage statusMessage;
	TStringConversion name = StringCast<ANSICHAR>(*InStatName);
	
	memcpy(statusMessage.StatName.data(), name.Get(), name.Length());
	statusMessage.Increasement = InStatIncreasement;
	FNetPlayerStateChannelCharacterStatusMessage::Send(NetDriver->ServerConnection,statusMessage);
}
